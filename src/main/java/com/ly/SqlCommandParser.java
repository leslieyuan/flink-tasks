...
public final class SqlCommandParser {
    private static final Logger log = LoggerFactory.getLogger(SqlCommandParser.class);
    ...
    public static List<SqlCommandCall> parseSql(String sql) {
        ...
        for (String s : sqlList) {
            log.info(s);
            Optional<SqlCommandCall> optionalCall = parse(s);
            if (optionalCall.isPresent()) {
                calls.add(optionalCall.get());
            } else {
                throw new RuntimeException("Unsupported command '" + s + "'");
            }
        }
        return calls;
    }
